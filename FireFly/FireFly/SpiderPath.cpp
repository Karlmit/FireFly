...
SpiderPath::SpiderPath(vector<sf::Vector2f> path)
	: mPath(path)
	, mLengths(path.size())
...
{	
	...
	Entity* jaktspindel = new JaktSpindel(1 ,mPath, mLengths, mDirections, mTotalLength);
	EntityList::getEntityList().addEntity(jaktspindel, Layer::Background);
	
}


void SpiderPath::updateEntity(sf::Time dt)
{	 
}

void SpiderPath::drawEntity(sf::RenderTarget& target, sf::RenderStates states) const
{
	// Draw Lines	
